...
def performance_cmd():
    ...
    ----------
    model : str
        Model to be used in the evaluation.
    file_name : str
        File where report is stored.
    sleep_time : int
        Frequency to check resource ussage. Higher frequency provides lower accuracy and
        process footprint.
    ...
    @ersilia_cli.command(
        help="""Generates a Hardware performance report on a selected model.
        e.g.,
    ...
    def performance(model, file_name="report.txt", sleep_time=10):
        ...
        proc = subprocess.Popen(
            f"python ./ersilia/cli/commands/utils/resource_monitor.py {sleep_time} {model} {file_name}",
            shell=True,
        )
        result = 0
        ...
        if result.exit_code != 0:
            click.secho(
                f"⛔️ Error when generating samples for the model: {model} ⛔️", fg="red"
            )
            return result
        # Run model
        result = __run_cli_tool(
            runner, run_cmd(), ["-imy_input.csv", "-omy_output.csv"]
        )
        if result.exit_code != 0:
